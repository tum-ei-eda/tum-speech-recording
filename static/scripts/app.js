        ...
        if (ajaxRequest.readyState == 4) {
          if (ajaxRequest.status === 200) {
            ...
            if (clipIndex < allClips.length) {
              ...
              allDone(ajaxRequest.responseText)
            }
          } else {
...
function allDone(path) {
  document.querySelector('.progress-display').innerText = 'Download successful.';
  ...
  record.disabled = false;
  record.innerText = 'Reset';
  document.querySelector('.fileslink').innerText = path;
  record.onclick = function() {
